...
void cache_level_init(cache_level *cache, unsigned int bank_size, unsigned int page_size, unsigned int counter_width)
{
	...
	cache->addr_arr = (unsigned int *)malloc(4 * cache->_pages);
...
{
	unsigned int counter = rlu_get_counter(cache, set) - 1;
...
{
	RET_STATUS res = HIT;
	unsigned int set = PART_NUM(addr, (cache->_offcet_width), cache->_set_width);
